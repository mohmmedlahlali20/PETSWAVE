...
import { Pets, PetsDocument } from '../pets/schema/pets.schema';
...
export class CommandesService {
  ...
  async create(commandsDTO: CreateCommandeDto): Promise<CommandeDocument> {
    ...
    if (
      commandsDTO.totalAmount !== undefined &&
      Math.abs(commandsDTO.totalAmount - totalAmount) > 0.01
    ) {
      throw new BadRequestException(
        'Total amount does not match the sum of pet prices',
      );
    }
